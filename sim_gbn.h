...
class SIM_GBN {
    ...
    void update_timeout(){
      this->timeout->time = this->buffer->get_head_time() + this->to;
    };

    double get_next_departure(){
      EventNode* temp = this->events->head;
      while(temp!=NULL && temp->event->event_type != DATA_DEPARTURE){
        temp = temp->next;
      }
      return temp->event->time;
    };

    void process_next_event(){
cout<<(*this->events)<<endl<<flush;
        ...
        switch(event->event_type){
            case TIMEOUT:
                //go back n
                if(this->sender->BUSY){
                  this->timeout->time = this->get_next_departure();
                  return;
                }
                this->timeout->time = this->tc + ((this->packet_length + this->header_length)/(double)this->link_rate) + this->to;
            ...
            case SEND_DATA:
                ...
                this->events->put_and_sort(new Event(this->tc + ((this->packet_length + this->header_length)/(double)this->link_rate), DATA_DEPARTURE, event->value, false));
                this->sender->BUSY = true;
                break;
            case DATA_DEPARTURE:
            ...
            case ACK_ARRIVAL:
                if(this->sender->BUSY){
                  event->time = this->get_next_departure();
                  this->events->put_and_sort(event);
